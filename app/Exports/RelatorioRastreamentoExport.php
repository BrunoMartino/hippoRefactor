...
class RelatorioRastreamentoExport implements FromArray, WithEvents, ShouldAutoSize, WithStyles
{
    public $request;
    ...
    public function array(): array
    {
        ...
        if ($request->has('t') && $request->t != '') :
            ...
            if ($request->has('t1') && $request->t1 != '')
                $types[] = 'RASTREIO - CODIGO';
            if ($request->has('t2') && $request->t2 != '')
                $types[] = 'RASTREIO - PROXIMO DESTINO';
            if ($request->has('t3') && $request->t3 != '')
                $types[] = 'RASTREIO - LOC ATUAL';
            if ($request->has('t4') && $request->t4 != '')
                $types[] = 'RASTREIO - SAIU ENTREGAR';
            if ($request->has('t5') && $request->t5 != '')
                $types[] = 'RASTREIO - CONFIRMACAO';
            if ($request->has('t6') && $request->t6 != '')
                $types[] = 'RASTREIO - AUSENTE';
        ...
        $billingsReport = $invoicingReport->orderBy('data_envio', 'desc')->get();
