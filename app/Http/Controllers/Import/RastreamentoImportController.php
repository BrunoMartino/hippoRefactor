...
use App\Imports\OrderImportRastreamento;

class RastreamentoImportController extends Controller
{
    ...
    {
        authorizePermissions(['criar-import-rastreamento']);
        return view('pages.config.import.rastreamento.imported_data.import');
    }

    public function importedData(Request $request)
    {
        authorizePermissions(['ver-import-rastreamento']);

        /* TODO: acho q aq vai ser os dados para  cada usuário, colocar o user_id para identificar */
        $data = ImportedOrderGroup::where('user_id', user_princ()->id)->where('module_id', 4);
        ...
        return view('pages.config.import.rastreamento.groups', compact('data'));
    }
    ...
    public function import(Request $request)
    {
        ...
        try {
            if ($this->hasWhatsAppRepeatedDifferentNames($request, $formato)):
                ...
                Excel::import(new OrderImportRastreamento($request->name), $request->file('arquivo'), null, ucfirst($formato));
            endif;
        } catch (\Throwable $th) {
            return redirect()->back()->withErrors([
        ...
        return redirect()->route('config.import.rt.imported-data')->withSuccess('Arquivo importado com sucesso!');
    }
    ...
    public function hasWhatsAppRepeatedDifferentNames($request, $formato): bool
    {
        ...
        try {
            ...
            $data = Excel::toArray(new OrderImportRastreamento($request->name), $request->file('arquivo'), null, ucfirst($formato));

            $collect1 = [];
            foreach (ImportedTracking::where('user_id', user_princ()->id)->get() as $item) {
                unset($item->id);
    ...
    public function show(Request $request, $group)
    {
        ...
        $data = $this->filterDataImported($request, $dataGroup->imported_tracking());
        // dd($data->toArray());
        return view('pages.config.import.rastreamento.imported_data.show', compact('data', 'dataGroup'));
    }
    ...
    {
        return Excel::download(new TesteRtExport, 'modelo-rastreamento-csv.csv', 'Csv');
        // return Excel::download(new TesteRtExport, 'modelo-rastreamento-xlsx.xlsx');
    }
    ...
    {
        authorizePermissions(['deletar-import-rastreamento']);

        if ($data->user_id != user_princ()->id)
            abort(403);

        ImportedTracking::where('group_id', $data->id)->delete();
        $data->delete();
    ...
    public function destroyItem(Request $request)
    {
        ...
        $data = ImportedTracking::where('user_id', user_princ()->id)->where('id', $request->id)->first();
        $data->delete();
