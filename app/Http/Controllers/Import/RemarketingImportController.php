...
use App\Imports\OrderImportRemarketing;
use Illuminate\Support\Facades\Session;

class RemarketingImportController extends Controller
{
    ...
    {
        authorizePermissions(['criar-import-remarketing']);
        return view('pages.config.import.remarketing.imported_data.import');
    }

    public function importedData(Request $request)
    {
        authorizePermissions(['ver-import-remarketing']);

        /* TODO: acho q aq vai ser os dados para  cada usuário, colocar o user_id para identificar */
        $data = ImportedOrderGroup::where('user_id', user_princ()->id)->where('module_id', 3);
        ...
        return view('pages.config.import.remarketing.groups', compact('data'));
    }
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            if ($this->hasWhatsAppRepeatedDifferentNames($request, $formato)):
                ...
                Excel::import(new OrderImportRemarketing($request->name), $request->file('arquivo'), null, ucfirst($formato));
            endif;
        } catch (\Throwable $th) {
            return redirect()->back()->withInput()->withErrors([
                'arquivo' => 'Errro ao importar arquivo'
            ]);
        }

        return redirect()->route('config.import.rm.imported-data')->withSuccess('Arquivo importado com sucesso!');
    }
    ...
    public function hasWhatsAppRepeatedDifferentNames($request, $formato): bool
    {
        ...
        try {
            ...
            $data = Excel::toArray(new OrderImportRemarketing($request->name), $request->file('arquivo'), null, ucfirst($formato));

            // $collect1= collect(ImportedBilling::where('user_id', user_princ()->id)->get()->toArray());
            $collect1 = [];
            foreach (ImportedRemarketing::where('user_id', user_princ()->id)->get() as $item) {
                unset($item->id);
    ...
    public function show(Request $request, $group)
    {
        ...
        $dataGroup = ImportedOrderGroup::where('user_id', user_princ()->id)
            ->where('id', $group)
            ->first();

        $data = $this->filterDataImported($request, $dataGroup->imported_remarketing());
        return view('pages.config.import.remarketing.imported_data.show', compact('data', 'dataGroup'));
    }
    ...
    {
        $data = ImportedOrderGroup::where('user_id', user_princ()->id)
            ->where('id', $group)
            ->first();
    ...
    {
        // return Excel::download(new TesteRmExport, 'modelo-remarketing-csv.csv', 'Csv');
        return Excel::download(new TesteRmExport, 'modelo-remarketing-xlsx.xlsx');
    }
    ...
    {
        authorizePermissions(['deletar-import-remarketing']);

        if ($data->user_id != user_princ()->id)
            abort(403);

        ImportedRemarketing::where('group_id', $data->id)->delete();
        $data->delete();
    ...
    public function destroyItem(Request $request)
    {
        ...
        $data = ImportedRemarketing::where('user_id', user_princ()->id)->where('id', $request->id)->first();
        $data->delete();
