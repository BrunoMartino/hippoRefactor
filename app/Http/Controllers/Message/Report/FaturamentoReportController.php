...
use App\Exports\RelatorioFaturamentoExport;
use Barryvdh\DomPDF\Facade\Pdf;

class FaturamentoReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('pages.messages.reports.faturamento.index', compact('totalMessagesSent', 'totalMessagesRemaining', 'reports', 'allReport'));
    }

    public function geAllReport()
    {
        $report = InvoicingReport::where('user_id', user_princ()->id)->get();
        return $report;
    }
    ...
    public function show(InvoicingReport $report)
    {
        ...
        return view('pages.messages.reports.faturamento.show', compact('report'));
    }
    ...
    public function totalMessagesSent(): int
    {
        ...
        $totalInvoicingReport = InvoicingReport::where('user_id', user_princ()->id)->count();

        return $totalReportMsg + $totalInvoicingReport;
    }
    ...
    {
        return Excel::download(new RelatorioFaturamentoExport($request), 'Relatório Faturamento ' . date('Y-m-d H.i.s') . '.xlsx');
    }

    public function exportPdf()
    {
        $reports = InvoicingReport::where('user_id', user_princ()->id)
            ->whereHas('message', function ($q) {
                return $q->where('module_id', 2);
            })->latest()->get();

        // return view('pages.messages.reports.pdf.faturamento');
        $pdf = Pdf::loadView('pages.messages.reports.pdf.faturamento', compact('reports'));
        return $pdf->download('Relatório Faturamento ' . date('Y-m-d H.i.s') . '.pdf');
        // return $pdf->stream('Relatório Faturamento ' . date('Y-m-d H.i.s') . '.pdf');
    }
}
