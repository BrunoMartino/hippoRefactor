    ...
    <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-2">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Importar Dados - Rastreamento</h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item">
                                <a class="text-muted text-decoration-none" href="{{ route('dashboard') }}">Home</a>
                            </li>
                            <li class="breadcrumb-item" aria-current="page">Importar Dados - Rastreamento</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body" id="content-01">
                    ...
                    <div class="alert alert-light text-dark px-2 py-1 mb-2 small mt-1 d-inline-block" role="alert"
                        ...
                        Os links que forem adicionados nos campos, devem ser links que permitam o download e acesso via
                        navegadores.
                    </div>
                    <div class="alert alert-light text-dark px-2 py-1 mb-2 small mt-1 fs-4 d-inline-block" role="alert"
                        style="background: #ffe8d2">
                        Para podermos rastrear os envios, necessário informar no campo Transportadora os nomes de acordo com
                        as transportadoras que temos integração.
                        <div class="mt-0">Os nomes estão em <a href="{{ route('config.sistema.rastreamento.index') }}"
                                class=" text-decoration-none text-dark fw-semibold">Configurações > Sistemas >
                                Rastreamento.</a></div>
                        <div class="mt-0">
                            Poderá informar qualquer transportadora que enviaremos o código de rastreio para seu cliente,
                            porém para rastreio somente as que temos integração.
                        </div>
                    </div>
                    <div class="mb-1">
                        Modelos de arquivos:
                        <a href="{{ asset('assets/files/modelo-rastreamento-xlsx.xlsx') }}"
                            class="btn btn-sm btn-light py-0" download>
                            <i class="ti ti-download"></i>
                            MODELO-RASTREAMENTO-XLSX
                        </a> e
                        <a href="{{ asset('assets/files/modelo-rastreamento-csv.csv') }}" class="btn btn-sm btn-light py-0"
                            download>
                            <i class="ti ti-download"></i>
                            MODELO-RASTREAMENTO-CSV
                        </a>.
                    </div>
                    <form action="{{ route('config.import.rt.store') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
...
@endsection
@section('scripts')
    @if (auth()->user()->hasPermissionTo('criar-import-rastreamento') == false)
        <script>
