                        ...
                        <div class="alert alert-warning mt-3 mb-4" role="alert">
                            ...
                            <div class="mt-1">
                                <button class="btn btn-orange btn-sm"
                                    onclick="document.getElementById('dt-especifica-data').required = false;document.getElementById('send_to_sales_from_date').removeAttribute('min'); document.getElementById('form').action= '{{ route('messages.sending-settings.redir-import-data', $message->id) }}';"
                                    type="submit">
                                    Importar Dados
                                </button>
                            </div>
                    ...
                    @else
                        <div class="row mt-4 mb-5">
                            @php
                                ...
                                if (is_array(old('groups_import', $dataSettings->use_imported_data_import))) {
                                    if (empty(old('groups_import', $dataSettings->use_imported_data_import))) {
                                ...
                                @php
                                    ...
                                    if (is_array(old('groups_import', $dataSettings->use_imported_data_import))) {
                                        if (
                                            in_array(
                                                $item->id,
                                                old('groups_import', $dataSettings->use_imported_data_import),
                                            )
                                        ) {
                                            $checkItemGroup = true;
                                        }
