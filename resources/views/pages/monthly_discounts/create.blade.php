    ...
    <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-2">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Novo Desconto</h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item" aria-current="page">Novo Desconto</li>
                        </ol>
                    </nav>
                </div>
            </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <x-alerts.warning/>
                    <!-- form -->
                    <form action="{{ route('descontos-mensais.store') }}" method="post">
                        @csrf
                        <div class="row gy-4">
                            <!-- cliente -->
                            <div class="col-12 col-lg-4">
                                ...
                                <div class="">
                                    <select id="cliente" class=" select-cliente  form-control custom-select2"
                                        ...
                                        @if (old('user_id'))
                                            <option value="{{ old('user_id') }}" data-select2-id="{{ old('user_id') }}">
                                                @php
                                                    $us = \App\Models\User::find(old('user_id'));
                                                @endphp
                                                @if ($us)
                                                    {{ $us->nome_usuario }}
                                                @endif
                            ...
                            <div class="col-12 col-lg-4">
                                ...
                                <div class="">
                                    <select id="modulo" class=" select-modulo  form-control custom-select2"
                                        ...
                                        @if (old('modulo_id'))
                                            <option value="{{ old('modulo_id') }}" data-select2-id="{{ old('modulo_id') }}">
                                                @php
                                                    $modu = \App\Models\Modulo::find(old('modulo_id'));
                                                @endphp
                                                @if ($modu)
                                                    {{ $modu->titulo }}
                                                @endif
                            ...
                            <div class="col-12 col-lg-4">
                                ...
                                <div class="">
                                    <select id="plano" class=" select-plano   form-control custom-select2"
                                        ...
                                        @if (old('plano_id'))
                                            <option value="{{ old('plano_id') }}"
                                                data-select2-id="{{ old('plano_id') }}">
                                                @php
                                                    $pla = \App\Models\Plano::find(old('plano_id'));
                                                @endphp
                                                @if ($pla)
                                                    {{ ucfirst($pla->nome) }}
                                                @endif
                                ...
                                <div class="d-flex gap-2 justify-content-between align-items-center">
                                    <div class="">
                                        <input type="text" class="form-control @error('valor') is-invalid @enderror"
                                            name="valor" id="valor" placeholder="0,00" value="{{ old('valor') }}"
                                            required />
                                        @error('valor')
                            ...
                            <div class="col-12 col-md-6 col-lg-4">
                                ...
                                <input type="month" class="form-control @error('dt_inicio') is-invalid @enderror"
                                    name="dt_inicio" id="dt_inicio" min="{{ date('Y-m') }}" placeholder=""
                                    value="{{ old('dt_inicio') }}" required />
                                @error('dt_inicio')
                                    <div class="invalid-feedback fw-semibold"> {{ $message }} </div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12 col-md-6 col-lg-4">
                                ...
                                <input type="month" class="form-control @error('dt_termino') is-invalid @enderror"
                                    name="dt_termino" id="dt_termino" min="{{ date('Y-m') }}" placeholder=""
                                    value="{{ old('dt_termino') }}" required />
                                @error('dt_termino')
                                    <div class="invalid-feedback fw-semibold"> {{ $message }} </div>
                                @enderror
                            </div>
                        ...
                        <div class="d-flex flex-column flex-sm-row gap-3 gap-sm-4 pt-5 mt-3">
                            ...
                            <a href="{{ route('descontos-mensais.index') }}" class="btn btn-light  px-5 fs-5 text-primary">
